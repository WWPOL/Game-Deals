...
class UserTask(models.Model):
    ...
    task_id = models.CharField(
        max_length=255, unique=True, db_index=True, help_text="Celery task ID"
    )
    task_name = models.CharField(
        max_length=255, help_text="Name of the task (e.g., 'deals.tasks.notify_deal')"
    )
    seen = models.BooleanField(
        default=False, help_text="Whether user has been notified of task completion"
    )
